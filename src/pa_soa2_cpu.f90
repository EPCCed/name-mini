...
#include "assertion.h"
  ...
  contains
    procedure :: printState        => pa_soa2_cpu_printState
    procedure :: releaseFromSource => pa_soa2_cpu_releaseFromSource
    procedure :: removeInactive    => pa_soa2_cpu_removeInactive
    procedure :: lastParticle      => pa_soa2_cpu_lastParticle
    procedure :: nameScheduleSize  => pa_soa2_cpu_nameScheduleSize
    procedure :: destroy           => pa_soa2_cpu_release
  end type pa_soa2_cpu_t

  public :: pa_soa2_cpu_create_pointer
  public :: pa_soa2_cpu_create
  public :: pa_soa2_cpu_release

  public :: pa_soa2_cpu_printState
  public :: pa_soa2_cpu_releaseFromSource
  public :: pa_soa2_cpu_removeInactive
  public :: pa_soa2_cpu_lastParticle
  public :: pa_soa2_cpu_nameScheduleSize

contains

  !---------------------------------------------------------------------------

  function pa_soa2_cpu_create_pointer(maxLocal) result(pa)

    ! Return pointer to a new object

    integer,               intent(in) :: maxLocal
    class (pa_soa2_cpu_t), pointer    :: pa

    allocate(pa)
    call pa_soa2_cpu_create(pa, maxLocal)

  end function pa_soa2_cpu_create_pointer

  !---------------------------------------------------------------------------

  subroutine pa_soa2_cpu_create(pa, maxLocal)

    ! Instantiate the manager object
    ! As this is cpu-only, there is no mapping to device memory

    type (pa_soa2_cpu_t), intent(out) :: pa
    ...
    call storage_soa2_create(pa%particles, pa%nmaxLocal, map = .false.)
  ...
  end subroutine pa_soa2_cpu_create

  !---------------------------------------------------------------------------

  subroutine pa_soa2_cpu_release(self)

    ! Release resources (no unmapping from device memory)

    class (pa_soa2_cpu_t), intent(inout) :: self
  ...
  end subroutine pa_soa2_cpu_release

  !---------------------------------------------------------------------------

  subroutine pa_soa2_cpu_releaseFromSource(self, nRelease, source)
    ...
    do iR = 1, nRelease
      ...
      iP  = pa_soa2_cpu_next_index(self)
      call storage_soa2_initialiseFromSource(self%particles, iP, iUP, source)

    end do
    !$omp end parallel do

  end subroutine pa_soa2_cpu_releaseFromSource

  !---------------------------------------------------------------------------

  subroutine pa_soa2_cpu_removeInactive(self)
  ...
  end subroutine pa_soa2_cpu_removeInactive

  !---------------------------------------------------------------------------

  subroutine pa_soa2_cpu_printState(self)

    ! Provide some information on the current state of the management

    class (pa_soa2_cpu_t), intent(in) :: self
  ...
  end subroutine pa_soa2_cpu_printState

  !---------------------------------------------------------------------------

  pure function pa_soa2_cpu_lastParticle(self) result(last)
  ...
  end function pa_soa2_cpu_lastParticle

  !---------------------------------------------------------------------------

  function pa_soa2_cpu_nameScheduleSize(self) result(nScheduleSize)
  ...
  end function pa_soa2_cpu_nameScheduleSize

  !---------------------------------------------------------------------------
  !---------------------------------------------------------------------------

  function pa_soa2_cpu_next_index(self) result(iP)

    type (pa_soa2_cpu_t), intent(inout)  :: self
    integer (int32)                   :: iP
  ...
  end function pa_soa2_cpu_next_index

  !---------------------------------------------------------------------------

  pure function pa_soa2_cpu_last_to_index(self, tid) result(iP)
    ...
    iBlock = pa_soa2_cpu_last_to_block(self, tid)
    iP     = 1 + iL + iBlock*NAME_PA_BLOCKSZ

    assert(0 <= iP .and. iP <= self%nmaxLocal)

  end function pa_soa2_cpu_last_to_index

  !----------------------------------------------------------------------------

  pure function pa_soa2_cpu_index_to_last(self, iP) result(last)
  ...
  end function pa_soa2_cpu_index_to_last

  !---------------------------------------------------------------------------

  pure function pa_soa2_cpu_last_to_block(self, tid) result(iblock)
  ...
  end function pa_soa2_cpu_last_to_block

  !---------------------------------------------------------------------------

  subroutine pa_soa2_cpu_update_last(self)
    ...
    do iP = 1, self%nmaxLocal
      ...
      self%last(tid) = pa_soa2_cpu_index_to_last(self, iP)
      lastBlock      = pa_soa2_cpu_last_to_block(self, tid)
    end do
    !$omp end do
    !$omp end parallel
  ...
  end subroutine pa_soa2_cpu_update_last

end module pa_soa2_cpu
